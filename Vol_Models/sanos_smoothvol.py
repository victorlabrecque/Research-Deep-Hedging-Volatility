...
def focus_line(
    ...
    *,
    ...
    if not focus is None:
        verify_inp(
            left < focus and focus < right,
            "'left', 'focus', and 'right' must be in order",
        )
    else:
    ...
    verify_inp(
        concentration >= 0.0 and concentration <= 1.0,
        "'concentration' must be within [0,1]",
    )

    x = np.linspace(-1.0, +1.0, N)
    x = x * (1.0 - concentration) + concentration * x * (
        np.pow(np.abs(x), power - 1) if power > 1 else 1.0
    )
    assert np.abs(x[0] + x[-1]) < 1e-8, ("Internal error", x[0], x[-1])
    x /= x[-1]
    x[x < 0.0] *= (left - focus) / x[0]
    x[x > 0.0] *= (right - focus) / x[-1]
    x += focus
    x[0] = left
    x[-1] = right
    return x
...
def vec_max_dx(
    ...
    * Strikes in the original strike range are at most ``max_dx`` apart from each other. If None, do not add strikes except min_k or max_k. Outside the extreme strikes of ``strikes```
    ...
    """
    verify_inp(
        min_k > 0.0 and min_k < 1.0 and max_k > 1.0,
        "'min_k' must be in (0,1) and 'max_k' must be above 1",
    )
    verify_inp(
        min_k < strikes[0] and max_k > strikes[-1],
        "'min_k' and 'max_k' must be outside 'strikes'",
    )
    verify_inp(max_dx is None or max_dx >= 0.0, "'max_dx' must be None or non-negative")
    verify_inp(min_dx >= 0.0, "'min_dx' must be non-negative")
    ...
    outer_max_dx = min(0.25, max_dx * 10.0)
    inner_max_dx = max_dx
    ...
    out = (
        np.linspace(
            min_k,
            left,
            max(2, 1 + math.floor((left - min_k) / outer_max_dx)),
            dtype=strikes.dtype,
        ).tolist()
        if left > min_k
        else [min_k]
    ...
    rout = (
        np.linspace(
            right,
            max_k,
            max(2, 1 + math.floor((max_k - right) / outer_max_dx)),
            dtype=strikes.dtype,
        ).tolist()
        if right < max_k
        else [max_k]
    ...
    six = (
        np.asarray(six, dtype=np.int32) if len(out) > len(strikes) + 2 else slice(1, -1)
    )
...
def _bs_call(k: np.ndarray, sqrtVar: np.ndarray):
    ...
    inix = (k <= 0.0) | (sqrtVar <= 0.0)
    calls[inix] = np.maximum(1.0 - k, 0.0)[inix]
    k = np.where(inix, 1.0, k)
    ...
    * $C^i_J := c^i_j/F_j/\mathrm{DF}_j$ are pure market prices.
    ...
    def __init__(
        ...
        *,
        ...
        self.intr = np.maximum(dtype(1.0) - self.strikes, dtype(0.0))
        self.spreads = self.asks - self.bids
        self.mids = 0.5 * (self.asks + self.bids)

        verify_inp(
            len(self.strikes.shape) == 1,
            lambda: f"Expiry {self.label}: 'strikes' must be a vector",
        )
        verify_inp(
            len(self.strikes) >= 2,
            lambda: f"Expiry {self.label}: must have at least two 'strikes'",
        )
        verify_inp(
            np.min(self.strikes[1:] - self.strikes[:-1]) > 0.0,
            lambda: f"Expiry {self.label}: 'strikes' must be increasing.",
        )
        verify_inp(
            self.strikes[0] > 0.0,
            lambda: f"Expiry {self.label}: 'strikes' must be positive",
        )
        verify_inp(
            self.strikes[0] < 1.0 and self.strikes[-1] > 1.0,
            lambda: (
                f"Expiry {self.label}: 'strikes' must surround 1. Found outer strikes {self.strikes[0]:.4g} and {self.strikes[-1]:.4g}"
            ),
        )
        verify_inp(self.sqrtT > 0.0, "'sqrt' must be positive")

        verify_inp(
            self.strikes.shape == self.bids.shape,
            lambda: (
                f"Expiry {self.label}: 'strikes' and 'bids' must have the same shape; found {self.strikes.shape} and {self.bids.shape}"
            ),
        )
        verify_inp(
            self.strikes.shape == self.asks.shape,
            lambda: (
                f"Expiry {self.label}: 'strikes' and 'asks' must have the same shape; found {self.strikes.shape} and {self.asks.shape}"
            ),
        )
        verify_inp(
            np.min(self.spreads) > 0.0,
            lambda: (
                f"Expiry {self.label}: 'bids' must be strictly below 'asks'; found minimum spread of {np.min(self.spread):.4g}"
            ),
        )
        verify_inp(
            np.max(self.asks - 1.0) < eps,
            lambda: (
                f"Expiry {self.label}: 'asks' must not exceed 1. Found {np.max(self.asks):.4g}"
            ),
        )
        verify_inp(
            np.min(self.bids - self.intr) >= -eps,
            lambda: (
                f"Expiry {self.label}: 'bids' must exceed instrinsic; found error {np.min(self.bids - self.intr):.4g}"
            ),
        )

        # floor bids and asks
        self.asks = np.minimum(self.asks, dtype(1.0))
        ...
        if self.iv_bids is None:
            ...
            verify_inp(
                self.iv_bids.shape == self.bids.shape,
                lambda: (
                    f"Expiry {self.label}: 'iv_bids' and 'bids' must have the same shape; found {self.iv_bids.shape} and {self.bids.shape}"
                ),
            )

        if self.iv_asks is None:
            self.iv_asks = bs_implied(
                self.strikes,
                self.asks,
                sqrtT=self.sqrtT,
                ret_only_vols=True,
            ...
            verify_inp(
                self.iv_asks.shape == self.asks.shape,
                lambda: (
                    f"Expiry {self.label}: 'iv_asks' and 'asks' must have the same shape; found {self.iv_asks.shape} and {self.asks.shape}"
                ),
            )
...
class ExpiryFit(ExpiryData):
    ...
    def _assign_result(self, *, jq, eps: float = 1e-8):
        ...
        assert jq.shape == self.xstrikes.shape, (
            "Shape error",
            jq.shape,
            self.xstrikes.shape,
        )
        err1 = min(np.min(jq), 0.0)
        err2 = np.abs(np.sum(jq) - 1.0)
        err3 = np.abs(jq @ self.xstrikes - 1.0)
        if err1 < -eps * 100.0 or err2 > eps * 100.0 or err3 > eps * 100.0:
            warnings.warn(
                f"Expiry {self.label}: output density has issues: "
        ...
        self.xdensity = np.maximum(0.0, jq)
        self.xfits, self.iv_xfits = self(self.xstrikes, and_vols=True)
        self.fits, self.iv_fits = self(self.strikes, and_vols=True)
        self.atm_call, self.atm_vol = self(1.0, and_vols=True)
        assert isinstance(self.atm_call, (np.number, float)), ("???", self.atm_call)
        self.atm_var = (self.atm_vol * self.sqrtT) ** 2

    def __call__(
        self, strikes: np.ndarray | float, *, and_vols: bool = False
    ) -> np.ndarray | float | tuple[np.ndarray, np.ndarray] | tuple[float, float]:
        """
        ...
        if isinstance(strikes, (float, int)):
            ...
            if and_vols:
                calls, vols = r
                assert len(calls) == 1 and len(vols) == 1, (
                    "Internal error",
                    calls,
                    vols,
                )
        ...
        calls = (
            _bs_call(
                strikes[:, None] / self.xstrikes[None, :],
                self.sqrtT * self.xvols[None, :],
            )
            * self.xstrikes[None, :]
        ) @ self.xdensity
        ...
        if not and_vols:
            return calls
        vols = bs_implied(
            strikes, calls, sqrtT=self.sqrtT, warn_only=True, ret_only_vols=True
        ).astype(self.dtype, copy=False)
        return calls, vols

    def _priceT(
        self, strikes: np.ndarray | float, *, sqrtT: float | None = None
    ) -> np.ndarray | float | tuple[np.ndarray, np.ndarray] | tuple[float, float]:
        """
        Compute call prices for ``strikes`` using the `SANOS <https://arxiv.org/abs/2601.11209>`_ model.
    ...
    def linear_market(
        self, strikes: np.ndarray, *, and_vols: bool = False
    ) -> np.ndarray:
        """
    ...
    def stats(self):
        ...
        * ``iv_bid_ask_err``: implied vol excess outside bid.ask, per strike
        """
        bid_ask_err = np.maximum(self.fits - self.asks, 0.0) + np.maximum(
            self.bids - self.fits, 0.0
        )
        return FitStats(
            bid_ask_err=bid_ask_err,
            bid_ask_rel_err=bid_ask_err / self.spreads,
            iv_bid_ask_err=np.maximum(self.iv_fits - self.iv_asks, 0.0)
            + np.maximum(self.iv_bids - self.iv_fits, 0.0),
        )
...
class FitStats(PrettyObject):
    ...
    class ErrStats(PrettyObject):
        def __init__(self, errors: np.ndarray | None = None):
            self.l1 = 0.0  # L1 error
            self.l2sq = 0.0  # L2 error^2
            self.el1 = 0.0  # L1 error of all errors
            self.el2sq = 0.0  # L2 error^2 of all errors
            self.max = 0.0  # Maximum error
        ...
        def __iadd__(self, errors=None):
            ...
            if isinstance(errors, type(self)):
                ...
                self.el1 = (
                    (self.el1 * self.nerr + errors.el1 * errors.nerr) / nnerr
                    if errors.nerr > 0
                    else self.el1
                )
                self.el2sq = (
                    (self.el2sq * self.nerr + errors.el2sq * errors.nerr) / nnerr
                    if errors.nerr > 0
                    else self.el2sq
                )
                ...
                if len(errors) == 0.0:
                    return self
                assert np.min(errors) >= 0.0, "'errors' cannot be negative"
                eerrors = errors[errors > 0.0]
                ...
                self.el1 = (
                    (self.el1 * self.nerr + np.sum(np.abs(eerrors))) / nnerr
                    if len(eerrors) > 0
                    else self.el1
                )
                self.el2sq = (
                    (self.el2sq * self.nerr + np.sum(eerrors)) / nnerr
                    if len(eerrors) > 0
                    else self.el2
                )
                self.max = (
                    max(self.max, np.max(eerrors)) if len(eerrors) > 0 else self.max
                )
                self.n = nn
                self.nerr = nnerr
            return self
...
class SmoothCallSurfaceConfig:
    ...
    bid_ask_mode: str = (
        "ignore"  # how to handle bid/ask: 'penalty', 'ignore', 'constraint'
    )
    ...
    max_iweight: float = 100.0  # maximum input weight, e.g. cap on 1/spread
    ...
    L1: bool = (
        True  # (not very useful) whether to use L1 for the fit objective vs mid, or L2
    )
    p_smoothness: float = 0.0  # (not very useful) if non-zero add a penalty term p_smoothness*mean(q^2) to the objective.

    @property
    def is_linear(self) -> bool:
        return self.vol_fac is None or self.vol_fac == 0.0
...
class SmoothCallSurface(object):
    ...
    def __init__(
        ...
        *,
        ...
        M = len(market)
        verify_inp(
            config.bid_ask_mode in ["ignore", "penalty", "constraint"],
            lambda: (
                f"'bid_ask' must be one of 'ignore', 'penalty', 'constraint' not '{config.bid_ask_mode}'"
            ),
        )
        ...
        if not config.model_strikes is None:
            verify_inp(
                len(config.model_strikes.shape) == 1 and len(config.model_strikes) >= 3,
                "'model_strikes' must be a vector with at least 3 entries",
            )
            verify_inp(
                config.model_strikes[0] > 0.0
                and config.model_strikes[0] < 1.0
                and config.model_strikes[-1] > 1.0
                and np.min(config.model_strikes[1:] - config.model_strikes[:-1]) > 0.0,
                "'model_strikes' must be a positive increasing vector, which surrounds 1",
            )
            ...
            if j > 0:
                if atm_vars[j] <= atm_vars[j - 1]:
                    ...
                    if raise_on_error:
                        raise ValueError(
                            err
                            + " -- use 'raise_on_error=False' to turn this error into a warning"
                        )
                    if not config.no_warnings:
                        warnings.warn(
                            err
                            + " -- ignored, but can cause infeasibility of the LP problem"
                        )
                jdata_m1 = market[j - 1]
                ixk = (jdata.strikes >= jdata_m1.strikes[0]) & (
                    jdata.strikes <= jdata_m1.strikes[-1]
                )
        ...
        for j in range(M):
            jdata = market[j]
            jr = ExpiryFit(jdata)
            assert min_k < jr.strikes[0] and max_k > jr.strikes[-1], (
            ...
            if config.model_strikes is None:
                jr.xstrikes, jr.mx_ix = vec_max_dx(
                    jr.strikes,
                    min_k=min_k,
                    max_k=max_k,
            ...
            if config.is_linear:
                ...
                verify_inp(
                    config.vol_mode == "market",
                    lambda: (
                        f"'vol_mode' must be 'atm', 'min', or 'market', not '{config.vol_mode}'"
                    ),
                )
                jr.xvols[1:-1] = config.vol_fac * jdata.interpolate(
                    jr.xstrikes[1:-1], "bid_vol"
                )
        ...
        # Solve
        # -----
        gerr = 0.0 if config.method == "global" else None
        ...
        for j in range(M):
            ...
            # martingale marginal
            jxq = cp.Variable(jxN)
            jconstraints = [jxq >= 0.0, cp.sum(jxq) == 1.0, jxq @ jr.xstrikes == 1.0]

            def jkernel(strikes):
                return (
                    (
                        _bs_call(
                            strikes[:, None] / jr.xstrikes[None, :],
                            jr.sqrtT * jr.xvols[None, :],
                        )
                        * jr.xstrikes[None, :]
                    )
                    if not config.is_linear
                    else np.maximum(jr.xstrikes[None, :] - strikes[:, None], 0.0)
                )
            ...
            if j == 0 or config.method == "individual":
                ...
                jr.xfloor = (
                    _bs_call(jr.xstrikes, jr.sqrtT * config.floor_vol)
                    if not config.is_linear
                    else np.maximum(1.0 - jr.xstrikes, 0.0)
                )
                ...
                assert jxC.shape == jr.xfloor.shape, (
                    "Internal error",
                    jxC.shape,
                    jr.xfloor.shape,
                )
                ...
                verify_inp(
                    ...
                    lambda: (
                        f"'method' must be 'global', 'iterative', or 'individual', not '{config.method}']"
                    ),
                )
                # global fit
                ...
                jxC_m1 = (
                    (
                        _bs_call(
                            jr.xstrikes[:, None] / jr_m1.xstrikes[None, :],
                            jr_m1.sqrtT * jr_m1.xvols[None, :],
                        )
                        * jr_m1.xstrikes[None, :]
                    )
                    @ jxq_m1
                    if not config.is_linear and not config.mart_theo
                    else np.maximum(jr_m1.xstrikes[None, :] - jr.xstrikes[:, None], 0.0)
                    @ jxq_m1
                )
            ...
            if config.bid_ask_mode == "constraint":
                if j > 0 and config.method == "iterative":
                    ...
                    floor = np.maximum(1.0 - jr.strikes, 0.0)
                ...
                if np.min(jr.bids - floor) < -eps:
                    ...
                    if not config.no_warnings:
                        warnings.warn(
                            err
                            + " -- ignored, but this can cause infeasibility of the LP problem"
                        )
            ...
            if config.spread_weighted:
                weight = np.minimum(dtype(config.max_iweight), 1.0 / jr.spreads)
                assert np.all(np.isfinite(weight)) and np.min(weight) > 0.0, (
                    "'weight' not positive and finite",
                    j,
                    jr.label,
                    "min weight",
                    np.min(weight),
            ...
            if config.bid_ask_mode == "penalty":
                ask_err = cp.maximum(0.0, jC - jr.asks)
                bid_err = cp.maximum(0.0, jr.bids - jC)
                ask_err = (
                    cp.multiply(weight, ask_err) if not weight is None else ask_err
                )
                bid_err = (
                    cp.multiply(weight, bid_err) if not weight is None else bid_err
                )
            ...
            if config.method == "global":
                ...
                if prob.status != "optimal":
                    ...
                    if prob.status != "optimal_inaccurate":
                        raise RuntimeError(
                            f"Expiry {jr.label}: solver {config.solver} failed : {prob.status} {status}"
                        )
                    warnings.warn(
                        f"Expiry {jr.label}: solver {config.solver} failed: {prob.status} {status}"
                    )
        ...
        if config.method == "global":
            ...
            if prob.status != "optimal":
                ...
                if prob.status != "optimal_inaccurate":
                    raise RuntimeError(
                        f"Solver {config.solver} failed for global fit: {prob.status} {status}"
                    )
                if not config.no_warnings:
                    warnings.warn(
                        f"Solver {config.solver} returned suboptimal global fit: {prob.status} {status}"
                    )
        ...
        for jr in results:
            stats += jr.stats
            atm_call, atm_vol = jr([1], and_vols=True)
        ...
        if len(results) > 2:
            Ts = np.array([0.0] + [jr.T for jr in results], dtype=dtype)
            Vs = np.array([0.0] + [jr.atm_var for jr in results], dtype=dtype)
    ...
    def __call__(self, strikes: np.ndarray | float, expiry: float):
        ...
        verify_inp(expiry >= 0.0, "'expiry' cannot be negative")
        ...
        atm_call = _bs_call(1.0, math.sqrt(atm_var))

        if expiry <= self.expiries[0]:
            lower_atm_call = 0.0
            lower_calls = np.maximum(1.0 - strikes, 0.0)
        else:
        ...
        assert lower_atm_call < upper_atm_call, (
            "ATM prices not increasing...?",
            lower_atm_call,
            upper_atm_call,
            lexp,
            uexp,
        ...
        calls = lower_calls * (1.0 - alpha) + upper_calls * alpha
        return calls if not is_float else calls[0]
    ...
    def plot(
        ...
        *,
        ...
        for j, jr in enumerate(self.results):
            with figure(
                f"{f'Smooth' if not self.is_linear else 'Linear'} fitted expiry {jr.label}"
                ...
                axCA, axC, axV, axD, axVD = fig.add_subplots(
                    "Calls (extended)" if show_CA else None,
                ...
                test_xstrikes = focus_line(
                    jr.xstrikes[0] * 0.9,
                    jr.xstrikes[-1] * 1.1,
                    num_focus_strikes,
                    power=10.0,
                    focus=0.5 * (jr.strikes[0] + jr.strikes[-1]),
                )
                test_strikes = focus_line(
                    jr.strikes[0], jr.strikes[-1], num_focus_strikes
                )
                ...
                txix = (jr.xstrikes >= jr.strikes[0]) & (
                    jr.xstrikes <= jr.strikes[-1]
                )  # map model strikes to market strike range

                # error bars
                C_spread = jr.asks - jr.bids
                C_err = (
                    np.maximum(jr.fits - jr.asks, 0.0)
                    + np.minimum(jr.fits - jr.bids, 0.0)
                ) / C_spread
                V_spread = jr.iv_asks - jr.iv_bids
                V_err = (
                    np.maximum(jr.iv_fits - jr.iv_asks, 0.0)
                    + np.minimum(jr.iv_fits - jr.iv_bids, 0.0)
                ) / V_spread

                # market
                market_xcalls = jr.linear_market(test_xstrikes)
                market_calls, market_vols = jr.linear_market(
                    test_strikes, and_vols=True
                )
                ...
                iv_xfloor = (
                    ...
                    if not jr.xfloor is None and plot_floor_vols
                    else None
                )
                ...
                if not axCA is None:
                    ...
                    axCA.plot(
                        jr.xstrikes, jr.xfits, ".", color="blue", label="fit (at target"
                    )
                    axCA.plot(
                        test_xstrikes,
                        test_xcalls,
                ...
                if not axC is None:
                    ...
                    axC.plot(
                        jr.strikes, jr.fits, ".", color="blue", label="fit (market)"
                    )
                    axC.plot(
                        jr.xstrikes[txix],
                        jr.xfits[txix],
                    ...
                    if C_err_max > 1e-6:
                        axC_2 = axC.twinx()
                        axC_2.plot(
                            jr.strikes, jr.strikes * 0.0, ":", color="blue", alpha=0.2
                        )
                        axC_2.bar(
                            jr.strikes,
                            C_err,
                ...
                def plot_vol(ax, vdetails=None):
                    ...
                    if not vdetails is None:
                        ...
                        verify_inp(
                            lo_k < hi_k,
                            lambda: (
                                f"'vdetails' lo_k {lo_k:.4g} must be below hi_k {hi_k:4.g}"
                            ),
                        )
                    ...
                    ax.plot(
                        jr.strikes, jr.iv_fits, ".", color="blue", label="fit (market)"
                    )
                    ax.plot(
                        test_strikes,
                        test_vols,
                    ...
                    if j > 0 and plot_prev_vols:
                        ax.plot(
                            jr.strikes, prev_vols, "2", color="orange", label="previous"
                        )
                    ...
                    if vdetails is None:
                        ...
                        ax.set_ylim(
                            m_o_m(
                                ...
                                jr.iv_fits,
                                test_vols,
                                iv_xfloor[txix] if not iv_xfloor is None else None,
                                prev_vols if j > 0 else None,
                            )
                        )
                    else:
                        min_dx = vdetails.get("min_dx", 0.0)
                        buf = vdetails.get("buf", 0.0)
                        ...
                        ax.set_ylim(
                            m_o_m(
                                ...
                                jr.iv_fits[det_ixs],
                                test_vols[det_ixs],
                                iv_xfloor[det_xixs] if not iv_xfloor is None else None,
                                prev_vols[det_ixs] if j > 0 else None,
                                min_dx=min_dx,
                                buf=buf,
                            )
                        )
                ...
                if not axD is None:
                    epsd = -1e-6
                    axD.plot(test_xstrikes, test_xstrikes * 0.0, ":", color="black")
                    axD.plot(
                        test_xstrikes[market_xp >= epsd],
                        market_xp[market_xp >= epsd],
                        "*:",
                        color="orange",
                        label="market",
                    )
                    axD.plot(
                        test_xstrikes[market_xp < epsd],
                        np.zeros_like(market_xp[market_xp < epsd]),
                        "*",
                        color="red",
                        label="market (negative)",
                    ) if sum(market_xp < 0.0) > 0 else None
                    axD.plot(
                        test_xstrikes, test_xq, ".-", color="blue", label="interpolated"
                    )

                    max_test_q = np.max(test_xq)
                    test_xix = (test_xstrikes >= jr.strikes[0] - fdx) & (
                        test_xstrikes <= jr.strikes[-1] + fdx
                    )
                    axD.set_xlim(
                        jr.strikes[0] - fdx, jr.strikes[-1] + fdx
                    ) if not density_x else None
                    axD.set_ylim(
                        m_o_m(
                            np.minimum(max_test_q * 2.0, market_xp[market_xp >= epsd]),
                            test_xq[test_xix],
                        )
                    ) if not density_x else None
                ...
                if out_path is not None:
                    ...
                    fig.savefig(
                        out_path / fname, silent_close=False, bbox_inches="tight"
                    )
