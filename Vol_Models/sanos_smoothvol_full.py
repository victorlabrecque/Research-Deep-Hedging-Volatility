...
def it_bs_kernel(strikes, BKx, BdVx, Bqx, prod_K, sum_dV, prod_q, fac):
    ...
    if m == 1:
        ky = prod_K * kx[None, :]
        return (
            nb_bs_call(strikes[:, None] / ky, fac * (sum_dV + dVx[None, :]))
            * ky
            * prod_q
        )
...
class SmoothComplexCallSurfaceConfig:
    ...
    p_smoothness: float = (
        0.0  # if non-zero add a penalty term p_smoothness*mean(q^2) to the objective.
    )
...
class SmoothComplexCallSurface:
    ...
    ** Experimental. Read the code and use it directly. This code plots prgress until it dies. **
    ...
    def __init__(
        ...
        M = len(market)
        verify_inp(
            config.bid_ask_mode in ["ignore", "penalty"],
            lambda: (
                f"'bid_ask' must be one of 'ignore' or 'penalty' not '{config.bid_ask_mode}'"
            ),
        )
        ...
        for j in range(M):
            ...
            jrvar_r_m1 = (
                ...
                if j == 0
                    ...
                    jr.xstrikes[1:-1], jr_m1.xstrikes[1:-1], jrvar_r_m1
                ).astype(dtype, copy=False)
            )
            jxvar_r = jr.interpolate(jr.xstrikes[1:-1], "bid_var")
            if np.min(jxvar_r - jrvar_r_m1) < 0.0:
                warnings.warn(
                    f"Expiry {jr.label}: interpolated variance not increasing; error {np.min(jxvar_r - jrvar_r_m1):.4g} -- will floor it."
                )
            jT = jr.sqrtT**2
            ...
            verify_inp(
                jT > jT_m1,
                lambda: (
                    f"Expiry {jr.label}: time to expiry is not increasing: found {jT:.4g} and {jT_m1:.4g}"
                ),
            )
            jr.xdV = np.zeros_like(jr.xstrikes)
            jr.xdV[1:-1] = np.maximum(
                jxvar_r - jrvar_r_m1, dtype((jT - jT_m1) * config.dvar_floor)
            ) * (config.vol_fac**2)
            jrvar_r_m1 = jxvar_r
        ...
        for j in range(M):
            ...
            print(
                f"\rProcessing expiry {j + 1}/{M} {jr.label} with {len(jr.strikes)} market strikes from {jr.strikes[0]:.4g} to {jr.strikes[-1]:.4g} (using {jxN} model strikes): generating tensors...",
                end="",
                flush=True,
            )
            ...
            KC = make_bs_kernel(
                jr.strikes, BK, BV, Bq, fac=1.0
            )  # kernel to map to model prices on market strikes
            KU = make_bs_kernel(
                jr.xstrikes, BK, BV, Bq, fac=0.0
            )  # kernel to map to X prices on model strikes
            tme_kernel = str(tme_kernel)
            print(
                f"\rProcessing expiry {j + 1}/{M} {jr.label} with {len(jr.strikes)} market strikes from {jr.strikes[0]:.4g} to {jr.strikes[-1]:.4g} (using {jxN} model strikes): produced kernel; this took {tme_kernel}. Now solving linear program... ",
                end="",
                flush=True,
            )
            ...
            # -----
            tme_solve = Timer()
            assert jxfloor_m1.shape == jr.xstrikes.shape, (
                "Internal error",
                jxfloor_m1.shape,
                jr.xstrikes.shape,
            )
            ...
            if prob.status != "optimal":
                ...
                if prob.status != "optimal_inaccurate":
                    raise RuntimeError(
                        f"Expiry {jr.label}: solver {config.solver} failed for expiry {jr.label}: {prob.status}{status}"
                    )
                warnings.warn(
                    f"Expiry {jr.label}: solver {config.solver} failed for expiry {jr.label}: {prob.status}{status}"
                )
            ...
            jr.iv_fits = bs_implied(
                jr.strikes, jr.fits, sqrtT=jr.sqrtT, warn_only=True, ret_only_vols=True
            ).astype(dtype, copy=False)
            jr.iv_Xcalls = bs_implied(
                jr.xstrikes,
                jr.Xcalls,
                sqrtT=jr.sqrtT,
                warn_only=True,
            ...
            tme_clean = Timer()
            print(
                f"\rProcessing expiry {j} DTE {jr.label} with {len(jr.strikes)} market strikes from {jr.strikes[0]:.4g} to {jr.strikes[-1]:.4g} (using {jxN} model strikes): solved linear program; this took {tme_solver}. Now cleaning up...",
                end="",
                flush=True,
            )
            KR = (
                make_bs_kernel(results[j + 1].xstrikes, BK, BV, Bq, fac=0.0)
                if j < M - 1
                else None
            )
            ...
            print(
                f"\rExpiry {j + 1}/{M} {jr.label} with {len(jr.strikes)} market strikes from {jr.strikes[0]:.4g} to {jr.strikes[-1]:.4g} (using {jxN} model strikes) done - timing: kernel {tme_kernel} solver {tme_solver} clean {tme_clean}. Memory: {get_memory_usage_mb() - init_mem}MB so far.",
                flush=True,
            )
