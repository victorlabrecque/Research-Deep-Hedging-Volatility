...
def bs_implied(
    ...
    if np.min(prices) < 0.0 or np.max(prices) >= 1.0:
        ...
        warnings.warn(err)
        prices = np.maximum(0.0, np.minimum(prices, 1.0))
    ...
    if len(prices.shape) == 2:
        ...
        strikes = (
            np.full(shape, strikes, dtype=strikes.dtype)
            if strikes.shape != shape
            else strikes
        )
        is_call = (
            np.full(shape, is_call, dtype=np.bool_)
            if is_call.shape != shape
            else is_call
        )
        sqrtT = (
            np.full(shape, sqrtT, dtype=sqrtT.dtype) if sqrtT.shape != shape else sqrtT
        )
        price_tol = (
            np.full(shape, price_tol, dtype=sqrtT.dtype)
            if price_tol.shape != shape
            else price_tol
        )
    ...
    with verbose.write_t(f"Computing implied vols for {total} options:") as tme:
        ...
        work &= ~done
        verbose.report(
            1, lambda: f"Used maximum volatility {vol_max:.3f} for {sum(done)} options."
        )
        ...
        work &= ~done
        verbose.report(
            1, lambda: f"Used minimum volatility {vol_min:.3f} for {sum(done)} options."
        )
        ...
        if not np.all(np.isfinite(vols)):
            verbose.report(
                1,
                lambda: (
                    f"**** Found {sum(~np.isfinite(vols))} infinite initial guess vols"
                ),
            )
        ...
        if sum(work) == 0:
            verbose.report(
                1, lambda: "*** No further implied volatilities to be computed ***"
            )
            ...
            for iters in range(1, max_iters + 1):
                ...
                if sum(work) == 0:
                    verbose.report(
                        2,
                        lambda: (
                            f"\rStep {iters}/{max_iters}: implied vol for the remaining {sum(done)} options computed."
                        ),
                        end="",
                    )
                    break
                res_max_err = np.max(
                    np.abs(fits[work] - prices[work]) / price_tol[work]
                )
                if tme.interval_test(0.5):
                    verbose.report(
                        2,
                        lambda: (
                            f"\rStep {iters}/{max_iters}: implied vol for {sum(done)} options computed; {sum(work)} options with a maximum error of {res_max_err:.4g} left to do."
                        ),
                        end="",
                    )
                ...
                new_vols = vols[work] + (prices[work] - fits[work]) / np.maximum(
                    test_vega, min_vega
                )
        ...
        verbose.write(
            lambda: (
                f"\rImplied vol calculation for {total} options finished using {err_stats.iters}/{err_stats.max_iters} iterations. "
    ...
    if isinstance(p_min, np.ndarray):
        if p_min.shape[0] == strikes.shape[0]:
            ...
            if not p_min.shape[0] == strikes.shape[0]:
                raise ValueError(
                    "'p_min' must have as many entries as strikes, or one less."
                )
    else:
        p_min = np.full(
            (strikes.shape[0] - 1,) + strikes.shape[1:], p_min, dtype=strikes.dtype
        )
