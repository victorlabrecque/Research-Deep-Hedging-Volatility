...
$$
where $q$ is the martingale density (the variable being optimized), $W$ is the total volatility of the BS-kernel (subject to a non-arbitrage constraint: the variance of the terminal distribution can only grow with horizon, never shrink), and $\hat{K}$, $\hat{T}$ are the surface grid, $\mu$ is the smoothing parameter. $C_{\text{\tiny SANOS}}$ is a complete option surface.
...
(\Sigma, W) \xrightarrow{\text{\tiny DLV}} (q, W) \xrightarrow{\text{\tiny SANOS}} C_{\text{\tiny SANOS}}
$$
...
Model a time series $h$ as follows (\textbf{baseline model, potential for further research}):
...
\textit{Simulation} (\textbf{base case example})
...
\begin{enumerate}
    ...
    \item Conditional on the complete surface path, generate log-spot
    $$
    ds_t = \mu \, dt + \beta^{\prime} \, dW_t^h + \varsigma \, dW_t^s
    $$
    where $\mu$ is the drift, $\beta$ represents the dependance with the option surface, and $\varsigma$ is the diffusion of the spot-specific shock. (e.g. $\mu$ and $\varsigma$ constant, $\beta=0$, is an independent Black-Scholes diffusion process)
\end{enumerate}
